...
# =============================================
# Module-Level Singletons (Alternative 2 Setup)
# =============================================
preprocessor = ImagePreprocessor()
onnx_model = OnnxModel()

# =============================================
# Deployment Entry Point (Cerebrium-Compatible)
# =============================================
def predict(item):
    """
    Cerebrium-compatible entry point that uses the singleton instances.
    Expects 'item' to contain a PIL Image under key 'image'.
    """
    try:
        # 1. Get PIL Image from input
        image = item.get("image")
        if not image:
            raise ValueError("Input must contain 'image' key with PIL Image")
            
        # 2. Preprocess using singleton
        processed_tensor = preprocessor.preprocess(image)
        
        # 3. Get logits from ONNX model
        logits = onnx_model.predict(processed_tensor)
